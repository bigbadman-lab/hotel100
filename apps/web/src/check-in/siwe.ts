...
export const CHECK_IN_CHALLENGE_STATEMENT = "Authorize HOTEL check-in eligibility.";
export const CHECK_IN_CHALLENGE_VERSION = "1";
const HEADER_SUFFIX = " wants you to sign in with your Ethereum account:";

export type ParsedCheckInChallenge = {
  domain: string;
  wallet: Address;
  uri: string;
  version: string;
...
export function buildCheckInChallengeMessage(input: {
  domain: string;
  wallet: Address;
  nonce: string;
  chainId: number;
  ...
  return [
    ...
    wallet,
    "",
    CHECK_IN_CHALLENGE_STATEMENT,
    "",
    `URI: ${challengeUri(input.domain)}`,
    `Version: ${CHECK_IN_CHALLENGE_VERSION}`,
...
export function parseCheckInChallengeMessage(message: string): ParsedCheckInChallenge | null {
  ...
  if (lines[2] !== "" || lines[3] !== CHECK_IN_CHALLENGE_STATEMENT || lines[4] !== "") return null;
...
export function messageMatchesIssuedChallenge(
  ...
  const expected = buildCheckInChallengeMessage({
    domain: row.domain,
    wallet,
    nonce: row.nonce,
